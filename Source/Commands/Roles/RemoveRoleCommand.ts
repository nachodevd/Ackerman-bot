...
export const command: Command = {
    name: "removerole",
    aliases: ['remove-role'],
    description: 'Con este comando podes removerle roles a un miembro.',
    category: 'Roles',
    enable: true,
    onlyOwner: false,
    use: ['remove-role <miembro> <rol>'],
    example: [],
    run: async (client, message, args) => {
        ...
        if (!member) return message.lineReply(new MessageEmbed().setDescription(`${client.Emojis.no_check} Falta de argumentos.\n\n|||removerole **<miembro>** <rol>||`).setColor(Discord))
        if (!role) return message.lineReply(new MessageEmbed().setDescription(`${client.Emojis.no_check} Falta de argumentos.\n\n|||removerole <miembro> **<rol>**||`).setColor(Discord))
        if(!member.roles.cache.get(role.id)) return message.lineReply(`${client.Emojis.no_check} El miembro no cuenta con ese rol.`)
        member.roles.remove(role.id).then((res) => {
            return message.lineReply(`${client.Emojis.check} Se removio el rol **${role.name}** de **${member.displayName}**.`)
        }).catch(() => {
